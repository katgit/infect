...
SD <- 9  # noise
MU <- 4  # mean of independent variables
...
as.dist(ppcor::pcor(DF |> dplyr::select(Xc,Yc,Zc_XcYc))$estimate)
#                 Xc         Yc
# Yc      -0.4917019           
# Zc_XcYc  0.6428611  0.7241640

## partial correlations for Xd --> Wc <-- Yc
as.dist(ppcor::pcor(DF |> dplyr::select(Xd,Yc,Wc_XdYc))$estimate)
#                 Xd         Yc
# Yc      -0.3214877           
# Wc_XdYc  0.4706841  0.7359997

## partial correlations for Xd --> Zd <-- Yd
as.dist(ppcor::pcor(DF |> dplyr::select(Xd,Yd,Zd_XdYd))$estimate)
#                 Xd         Yd
# Yd      -0.0178907           
